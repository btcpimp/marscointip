# Reddit Marscointip bot

## Introduction

For introduction to and use of Marscointip bot, see __http://www.reddit.com/r/Marscointip/wiki/index__

## Getting Started

>**NEW: See [Marscointip-chef](https://github.com/vindimy/Marscointip-chef) for Chef-automated installation and configuration of Marscointip bot.**

### Python Dependencies

The following Python libraries are necessary to run Marscointip bot:
...
Create a new MySQL database instance and run included SQL file [Marscointip.sql](Marscointip.sql) to create necessary tables. Create a MySQL user and grant it all privileges on the database. If you don't like to deal with command-line MySQL, use `phpMyAdmin`.
...
Here's the first few lines of DEBUG-level console output during successful initialization.
    ...
    user@host:/opt/Marscointip/Marscointip/src$ ./_start.sh
    INFO:cointipbot:CointipBot::init_logging(): -------------------- logging initialized --------------------
    DEBUG:cointipbot:CointipBot::connect_db(): connecting to database...
    INFO:cointipbot:CointipBot::connect_db(): connected to database Marscointip as Marscointip
    DEBUG:cointipbot:CtbCoin::__init__(): connecting to Peercoin...
    DEBUG:bitcoin:Read 5 parameters from /opt/Marscointip/coins/ppcoin/ppcoin.conf
    ...
    DEBUG:bitcoin:Read 5 parameters from /opt/Marscointip/coins/primecoin/primecoin.conf
    ...
    DEBUG:bitcoin:Read 5 parameters from /opt/Marscointip/coins/megacoin/megacoin.conf
    ...
    DEBUG:bitcoin:Read 5 parameters from /opt/Marscointip/coins/litecoin/litecoin.conf
    ...
    DEBUG:bitcoin:Read 5 parameters from /opt/Marscointip/coins/namecoin/namecoin.conf
    ...
    DEBUG:bitcoin:Read 5 parameters from /opt/Marscointip/coins/bitcoin/bitcoin.conf
    ...
    INFO:cointipbot:CointipBot::connect_reddit(): logged in to Reddit as Marscointip
    ...

Marscointip bot is configured by default to append INFO-level log messages to `logs/info.log`, and WARNING-level log messages to `logs/warning.log`, while DEBUG-level log messages are output to the console.
...
There are three simple backup scripts included that support backing up database, wallets, and configuration files to local directory and (optionally) to a remote host with `rsync`. Make sure to schedule regular backups with cron and test whether they are actually performed. Example cron configuration:
    ...
    0 8,20 * * * cd /opt/Marscointip/Marscointip/src && python _backup_db.py ~/backups
    0 9,21 * * * cd /opt/Marscointip/Marscointip/src && python _backup_wallets.py ~/backups
    0 10 * * * cd /opt/Marscointip/Marscointip/src && python _backup_config.py ~/backups

### Cron: Statistics

Marscointip bot can be configured to generate tipping statistics pages (overall and per-user) and publish them using subreddit's wiki. After you configure and enable statistics in configuration, add the following cron job to update the main statistics page periodically:

    0 */3 * * * cd /opt/Marscointip/Marscointip/src && python _update_stats.py
